...
public class TestAuth
{
    ...
    public void vSphereLogin()
    {
        ...
        Promise<List<ObjectContent>> vSphereObject = testPromises.flatMap((VimPortType vimPortType) -> {
            // VSphereObject vsphere  = new VSphereObject();
            try
            {
                return Promise.pure(getDatacenters());
            }
            catch (Exception e)
            {
                // TODO Auto-generated catch block
        ...
        vSphereObject.onComplete((List<ObjectContent> list) -> {
            ...
            {
                ManagedObjectReference dcRef = datacenter.getObject();
        ...
        // final CompletableFuture<VimService> future = CompletableFuture.supplyAsync(() -> {
        /*  Promise<VimPortType> testPromise = Promise.promise(WorkReactor.getDefaultWorkReactor(), new Function0<VimPortType>() {
            @Override
            public VimPortType exec()
            {
                //...long running...
                ...
                {
                    UserSession session =
                            vimPortType.login(serviceContent.getSessionManager(), "us\\rnunna", "Good1@good2", null);
        ...
        System.out.println("vSphere vimservice onCompletable");
        //This is callback feature
        // future.thenAccept(vim -> print(vim)); //This will not block thread. Once we response, this will be invoked.
        testPromise.onComplete(vim -> print(vim));
        System.out.println("done accept");*/
        /* try
        {
            System.out.println("isdone" + future.get()); //This will block the thread untill we get response
        }
    ...
    /*  private void getDataCenters(Promise<List<ObjectContent>> vSphereObject2, WorkReactor workReactor)
    {
        // TODO Auto-generated method stub
        for (ObjectContent vSphereObject : vSphereObject2)
        {
    ...
    private Object print(VimPortType vim)
    {
        // TODO Auto-generated method stub
        System.out.println("vimservice" + vim);
        vimPortType = vim;
        return vim;
    }

    private List<ObjectContent> getDatacenters() throws InvalidPropertyFaultMsg, RemoteException, RuntimeFaultFaultMsg, Exception
    {
        ...
        RetrieveResult results = getVimPortTypeService().retrievePropertiesEx(content.getPropertyCollector(), specs, options);
